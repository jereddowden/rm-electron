(function () {
    ...
    unpackAll.unpack = function unpack(archiveFile, options, callback) {
        ...
        if (!unar) {
            if (isWindows()) {
                unar = path.resolve(path.basename(__dirname),'bin/unar.exe');
            } else {
                unar = path.resolve(path.basename(__dirname),'bin/unar');
            }
        }
        var ar = [unar];
    ...
    unpackAll.list = function list(archiveFile, options, callback) {
        ...
        if (!lsar) {
            if (isWindows()) {
                lsar = path.resolve(path.basename(__dirname),'bin/lsar.exe');
            } else {
                lsar = path.resolve(path.basename(__dirname),'bin/lsar');
            }
        }
        var ar = [lsar];
